...
public abstract class ChartData<T extends IDataSet<? extends Entry>> {
    ...
    public float getYMin(AxisDependency axis) {
        if (axis == AxisDependency.LEFT) {
            return mLeftAxisMin == Float.MAX_VALUE ? mRightAxisMin : mLeftAxisMin;
        } else {
            return mRightAxisMin == Float.MAX_VALUE ? mLeftAxisMin : mRightAxisMin;
        }
    }
    ...
    public float getYMax(AxisDependency axis) {
        if (axis == AxisDependency.LEFT) {
            return mLeftAxisMax == -Float.MAX_VALUE ? mRightAxisMax : mLeftAxisMax;
        } else {
            return mRightAxisMax == -Float.MAX_VALUE ? mLeftAxisMax : mRightAxisMax;
        }
    }
    ...
    public Entry getEntryForHighlight(Highlight highlight) {
        if (highlight.getDataSetIndex() >= mDataSets.size()) return null;
    ...
    public T getDataSetByIndex(int index) {
        ...
        if (mDataSets == null || index < 0 || index >= mDataSets.size()) return null;
    ...
    public void addEntry(Entry e, int dataSetIndex) {
        if (e == null || dataSetIndex < 0 || dataSetIndex >= mDataSets.size()) return;
    ...
    protected void calcMinMax(Entry e, AxisDependency axis) {
        ...
        if (mXMax < x) mXMax = e.getX();
        if (mXMin > x) mXMin = e.getX();

        if (axis == AxisDependency.LEFT) {
            if (mLeftAxisMax < y) mLeftAxisMax = y;
            if (mLeftAxisMin > y) mLeftAxisMin = y;
        } else {
            if (mRightAxisMax < y) mRightAxisMax = y;
            if (mRightAxisMin > y) mRightAxisMin = y;
        }
    }

    protected void calcMinMax(T d) {

        final float xMax = d.getXMax(),
                xMin = d.getXMin(),
                yMax = d.getYMax(),
                yMin = d.getYMin();

        if (mYMax < yMax) mYMax = yMax;
        if (mYMin > yMin) mYMin = yMin;

        if (mXMax < xMax) mXMax = xMax;
        if (mXMin > xMax) mXMin = xMin;

        if (d.getAxisDependency() == AxisDependency.LEFT) {
            if (mLeftAxisMax < yMax) mLeftAxisMax = yMax;
            if (mLeftAxisMin > yMin) mLeftAxisMin = yMin;
        } else {
            if (mRightAxisMax < yMax) mRightAxisMax = yMax;
            if (mRightAxisMin > yMin) mRightAxisMin = yMin;
        }
    }
    ...
    public int[] getColors() {
        ...
        for (int i = 0, size = mDataSets.size(); i < size; i++) {
            colorCount += mDataSets.get(i).getColors().size();
        }
        ...
        for (int i = 0; i < mDataSets.size(); i++) {
            ...
            List<Integer> setColor = mDataSets.get(i).getColors();

            for (Integer clr : setColor) {
                colors[cnt] = clr;
                cnt++;
            }
        }
    ...
    public void setValueTextSize(float size) {
        ...
        for (int i = 0, count = mDataSets.size(); i < count; i++) {
            T set = mDataSets.get(i);
     ...
     * if not.
     *
     * @return is hightlight enabled or not
     */
    ...
    /**
     * Returns the total entry count of all the data sets.
     *
     * @return the count sum of all the entries.
     */
    ...
    /**
     * Get the {@link DataSet} object with the maximum number.
     *
     * @return the set with maximum-count items.
     */
